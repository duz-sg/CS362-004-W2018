...
int checkSmithyCard(int p, struct gameState *post, int handPos) {
  ...
  printf ("smithy PRE: p %d HC %d DeC %d DiC %d Played %d Deck: %d, %d, %d \n",
  p, pre.handCount[p], pre.deckCount[p], pre.discardCount[p],
  pre.playedCardCount,
  pre.deck[p][pre.deckCount[p]-1],
  pre.deck[p][pre.deckCount[p]-2],
  ...
  int r = smithyEffect(post, handPos, p);

  printf ("smithy POST: p %d HC %d DeC %d DiC %d Played %d Hand: %d, %d, %d \n",
  p, post->handCount[p], post->deckCount[p], post->discardCount[p],
